...
export function PlanOverview({ plan, done, onToggle, onReset, todayIso: _todayIso }: Readonly<{
  plan: StudyPlan;
        ...
        <section className={sessionStyles.planWeek} key={week.startsOn} aria-labelledby={`plan-week-${weekIndex}`}>
          ...
          <ul className={sessionStyles.planChecklist}>
            {week.items.map((item, itemIndex) => {
              ...
              return <li key={key}>
                <input id={id} type="checkbox" checked={done[key] ?? false} onChange={(event) => onToggle(key, event.target.checked)} />{' '}
                <label htmlFor={id}>{MODE_LABEL[item.mode] ?? item.mode} · {scenario}</label>
                <Link href={`/learn/${plan.courseSlug}?concept=${item.conceptId}`} aria-label={`Open lesson: ${scenario}`}>Study<span aria-hidden="true"> →</span></Link>
              </li>;
            })}
          </ul>
        </section>
