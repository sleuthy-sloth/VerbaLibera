...
Open **New: structured A1 foundations with offline study** from the Italian/French lesson index. Use **Download for offline study**, then open/bookmark the provided offline-study link. The shared offline workspace supports teaching, local grading, downloaded audio and durable device practice without a connection. Every foundation lesson now has a prerecorded model and optional dictation with slow replay. The bottom **Listen** tab ships audio-only Thinking Method tracks for walks and screen-off study: French lesson 1 has a 2-minute track with think-pauses and French reveals, and the Italian foundations market lesson now has a 7.5-minute long-form track — *Al mercato* — where an English teacher recombines numbers, prices by the kilo, and polite requests across two market dialogues, with Italian reveals and 10s/5s/2.5s think-pauses (44 locally generated clips; per-clip faster-whisper QA recorded in `docs/audio-provenance/italian-market-listen.json`). Finishing a track logs it heard, never mastered. The local Kokoro recordings passed waveform checks and transcription pre-screening; native-speaker prosody review remains open. Listening practice does not reset completed text lessons. The Quiet Ink interface ships original brand artwork: logo mark and lockup, hero banner, per-language course banners and a day-zero journal illustration (`public/brand/`).
