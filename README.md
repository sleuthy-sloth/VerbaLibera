...
This checkout includes foundation packs for French, German, Italian, Portuguese, and Spanish. French and Italian contain the broadest curricula. German, Brazilian Portuguese, and Spanish now each have a four-lesson route: first words, introductions, café requests, and numbers and quantities. The visible **Course path** marks completed lessons for review, highlights the next available lesson, and explains what unlocks each later lesson. French and Italian both open with a words-first lesson (greetings and Latin-shared words) before any sentence building. Each lesson teaches a pattern before asking you to use it: the lesson page shows aim, explanation, worked examples and a word list, practice opens on a zero-recall meet-the-word choice, and the lesson model autoplays when practice begins so the learner hears the pattern first. Grammar references and vocabulary views are linked to practice evidence. These are partial A1 courses, not full CEFR programs or certifications; native-speaker editorial review remains open.

Open **New: structured A1 foundations with offline study** from the Italian/French lesson index. Use **Download for offline study**, then open/bookmark the provided offline-study link. The shared offline workspace supports teaching, local grading, downloaded audio and durable device practice without a connection. Every foundation lesson now has a prerecorded model and optional dictation with slow replay. The bottom **Listen** tab ships audio-only Thinking Method tracks for walks and screen-off study: French lesson 1 has a 2-minute track, Italian has the 7.5-minute *Al mercato* lesson, and Spanish, Brazilian Portuguese, and German each have a 10-minute introductions-and-café preview with think-pauses and target-language reveals. The new tracks can be saved as MP3s for offline playback outside the app; their dedicated Listen screen is not yet part of a fresh offline app launch. Per-track provenance and automated transcription review live in `docs/audio-provenance/`; native-speaker language and prosody review remains open. Finishing a track logs it heard, never mastered. Listening practice does not reset completed text lessons. The Quiet Ink interface ships original brand artwork: logo mark and lockup, hero banner, per-language course banners and a day-zero journal illustration (`public/brand/`).
...
VerbaLibera is in active development. `v0.2.0` adds guided long-form listening previews in Spanish, Brazilian Portuguese, and German; extends each of those foundations through numbers and quantities; and introduces a course path that makes next lessons, prerequisites, and review status visible. The tag-triggered release job publishes the portable HTML and unsigned Apple Silicon DMG with their SHA-256 manifests.
