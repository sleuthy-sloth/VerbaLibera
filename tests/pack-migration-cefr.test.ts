 ...
 * nothing. Since German, Portuguese and Spanish are still v1 and would have lost
 * their tags the same way, the field exists now and the migration carries it.
 *
 * This runs on the packs still at v1 (no manifest is written) so the guarantee
 * is proven against real content, not a fixture.
 */

const V1_LANGUAGES = ["german", "portuguese", "spanish"] as const;
...
describe("the v1→v2 migration keeps the authored CEFR tag", () => {
  ...
  it("a v2 pack authored without tags stays untagged rather than inventing a level", () => {
    ...
    const raw = readPack("german") as { lessons: Array<Record<string, unknown>> };
  ...
  it("rejects a level the schema does not claim (the field is a claim, not a label)", () => {
    const migrated = migratePackV1ToV2(readPack("german")) as { lessons: Record<string, unknown>[] };
