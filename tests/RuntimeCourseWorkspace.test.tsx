...
it('opens a v2 story through the course boundary while keeping prerequisites locked',async()=>{
 ...
 await waitFor(() => expect(screen.getByRole('button',{name:'Start Una storia al bar'})).toBeEnabled());
 expect(screen.getByRole('button',{name:'Start Al bar con il barista'})).toBeDisabled();
 await userEvent.click(screen.getByRole('button',{name:'Start Una storia al bar'}));
