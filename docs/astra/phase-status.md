...
| 2 Content architecture | Versioned JSON packs, schema and references implemented; richer level/stage/skill metadata remains partial. French and German migrated from schemaVersion 1 to 2 on 2026-09-10 through `migratePackV1ToV2()` with identity parity, history replay and a flip rehearsal proven by test; Portuguese and Spanish remain v1. The v2 lesson shape now carries the optional `cefr` and `culturalNote` fields the migration used to drop, so German kept all 8 authored tags and notes; French and Italian predate the fields and report none (a data decision, not a silent loss). |
...
| 13 Pronunciation | Existing optional local transcription retained; no invented pronunciation score. Italian is the only course with speaking steps (23). Portuguese and Spanish are schemaVersion 1 and need a player capability before content; French and German run the v2 player and could carry speaking steps, but none are authored. |
