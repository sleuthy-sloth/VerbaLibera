...
| 2 Content architecture | Versioned JSON packs, schema and references implemented; richer level/stage/skill metadata remains partial. All five packs migrated from schemaVersion 1 to 2 through `migratePackV1ToV2()` with identity parity, history replay and a flip rehearsal proven by test; Spanish went last, and with it the tree stopped holding a real v1 pack, so the v1 engine's suites now host on `tests/fixtures/lesson-variety.ts` (a real three-lesson A1 Spanish starter). The v2 lesson shape now carries the optional `cefr` and `culturalNote` fields the migration used to drop, so German kept all 8 authored tags and notes; French and Italian predate the fields and report none (a data decision, not a silent loss). |
| 3 Curriculum | French and Italian: 25 lessons each (L0 words-first opener + Units 1–6); partial A1. German, Portuguese and Spanish ship as active starter packs (8 lessons each, v0.4.0, 48–49 practice activities). More domains and depth remain. Native-speaker review of the authored prose is closed for German and Spanish (2026-09-11, recorded in `courses/<language>/review.json` and reflected in the generated reports); it is still open for French, Italian and Portuguese, and the audio listening review is open for every course. See `docs/human-review-gates.md`. |
...
| 13 Pronunciation | Existing optional local transcription retained; no invented pronunciation score. Italian is the only course with speaking steps (23). Every course runs the v2 player now, so the other four could carry speaking steps; none are authored. |
